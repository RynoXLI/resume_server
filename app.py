...
message_template = "Someone sent a message!\n" \
                               "First Name: ${fname};\n" \
                               "Last Name: ${lname};\n" \
                               "Email Address: ${email};\n" \
                               "Message: ${msg};\n" \
...
def home():
    now = datetime.now()
    return render_template('index.html', time=now, base=conf['base'], conf=conf['index'])
...
def message():
    ...
    if request.method == "POST" and form.validate():
        ...
        if form.cc.data:
            ...
            outputText = template.render(fname=form.fname.data.title(), msg=form.msg.data, pub_time=formated_time)  # this is where to put args to the template renderer
        ...
        plain_msg = message_template.substitute(fname=form.fname.data,
